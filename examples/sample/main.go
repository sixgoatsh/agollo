...
func main() {
	...
	err := agollo.InitWithDefaultConfigFile(
		options.WithLogger(log.NewLogger(log.LoggerWriter(os.Stdout))), // 打印日志信息
		options.PreloadNamespaces("TEST.Namespace"),                    // 预先加载的namespace列表，如果是通过配置启动，会在app.properties配置的基础上追加
		options.AutoFetchOnCacheMiss(),                                 // 在配置未找到时，去apollo的带缓存的获取配置接口，获取配置
		options.FailTolerantOnBackupExists(),                           // 在连接apollo失败时，如果在配置的目录下存在.agollo备份配置，会读取备份在服务器无法连接的情况下
	)
	if err != nil {
		panic(err)
	}
	...
	*/
	...
	fmt.Println("YourConfigKey:", agollo.Get("YourConfigKey", options.WithDefault("YourDefaultValue")))

	// 获取默认配置中cluster=default namespace=Test.Namespace key=timeout的值，提供默认值返回
	fmt.Println("YourConfigKey2:", agollo.Get("YourConfigKey2", options.WithDefault("YourDefaultValue"), options.WithNamespace("YourNamespace")))
	...
	// 未配置的话会返回空或者传入的默认值选项
	fmt.Println(agollo.Get("timeout", options.WithDefault("foo"), options.WithNamespace("TEST.Namespace1")))
