...
var (
	...
	// key: endpoint value: agollo.GoApollo
	agolloMap sync.Map
)
...
var (
	...
	// 默认创建Agollo的Option
	defaultAgolloOptions = []options.Option{
...
func SetAgolloOptions(opts ...options.Option) {
...
type apolloConfigManager struct {
	agollo agollo.GoApollo
}

func newApolloConfigManager(appid, endpoint string, opts []options.Option) (*apolloConfigManager, error) {
	if appid == "" {
...
func newAgollo(appID, endpoint string, opts []options.Option) (agollo.GoApollo, error) {
	i, found := agolloMap.Load(agolloKey(appID, endpoint))
	if !found {
		innerOpts, err := options.NewOptions(endpoint, appID, opts...)
		if err != nil {
			return nil, err
		}
		newBalancer, err := balancer.NewBalancer(innerOpts.Conf, innerOpts.EnableSLB, innerOpts.RefreshIntervalInSecond, innerOpts.Logger, &client.MetaServerClient{})
		if err != nil {
			return nil, err
		}

		ag, err := agollo.NewGoApollo(
			endpoint,
			appID,
			client.New(),
			newBalancer,
		...
		agolloMap.Store(agolloKey(appID, endpoint), ag)

		return ag, nil
	}
	return i.(agollo.GoApollo), nil
}
